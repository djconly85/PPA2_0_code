...
def dissolve_x_hex(fc_intersect_w_propn, dict_fld_pcl_vals, fc_hexs, fld_hexid,
    ...
    outfields_dict = dict(zip(wtdval_fields, output_wtdav_fields))
    ...
    for field in wtdval_fields:
        ...
        curfields = curfields + [area_pcl] # need to manually add area field (normally SHAPE@AREA)
        ...
        with arcpy.da.UpdateCursor(fc_hexoutput, curfields) as cur:
            for row in cur:
                ...
                area = row[curfields.index(area_pcl)]
...
if __name__ == '__main__':
    ...
    fc_hexoutput = "hex_w_wtdavgs_{}".format(date_sufx)
