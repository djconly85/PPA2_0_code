...
def calc_mix_index(in_df, params_csv, hh_col, lu_factor_cols):
    ...
    in_df['mix_index_halfmi'] = in_df.swifter.apply(lambda x: get_wtd_idx(x, lu_facs, params_df), axis = 1)
...
def do_work(in_csv, out_csv, params_csv, input_cols, landuse_cols, col_k12_enr, 
    ...
    print("calculating mix index...")
    out_df = calc_mix_index(parcel_df, params_csv, col_hh, lu_fac_cols)

    out_df[[col_parcelid, col_hh, 'mix_index_halfmi']].to_csv(out_csv, index = False)
...
if __name__ == '__main__':
    ...
    in_pcl_csv = r"I:\Projects\Darren\PPA_V2_GIS\SACSIM Model Data\parcel_16_buf_flat_10292019.txt"
    
    # weighting values {land use:[optimal ratio per household, weight given to that ratio]}
    mix_idx_params_csv = r"Q:\ProjectLevelPerformanceAssessment\PPAv2\PPA2_0_code\PrepDataInputs\mix_idx_params_halfmi.csv"
    
    # output csv for testing
    out_id = int(round(time.time(),0))
    out_csv = r'C:\Users\dconly\PPA_TEMPFILES\test_mixindex_out{}.csv'.format(out_id)
    ...
    print("time elapsed: {}mins".format(elapsed))
