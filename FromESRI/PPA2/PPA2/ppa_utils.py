...
class Publish(object):
    def __init__(self, in_df, xl_template, import_tab, xl_out, xlsheets_to_pdf=None, map_key_csv=None, 
        ...
        self.sheets_all_rpts = p.sheets_all_reports
    ...
    def make_pdf(self):
        wb = None
        ok = p.C_OK
        out_pdf_final_name = "Report{}.pdf".format(int(time()))
        out_pdf_final = os.path.join(p.dir_pdf_output, out_pdf_final_name)
        if(os.path.exists(out_pdf_final)):
            try:
                os.remove(out_pdf_final)
            except:
                out_pdf_final = os.path.join(p.dir_pdf_output, "xpdf{}.pdf".format(int(time.clock())))
        ...
        try:
            ...
            #..Calling the append pdf function
            pdf_final = arcpy.mp.PDFDocumentCreate(out_pdf_final)
            for out_pdf in l_out_pdfs:
                pdf_final.appendPages(out_pdf)

            pdf_final.saveAndClose() 
            #  dopdf(l_out_pdfs, out_pdf_final) -> resulting pdf
            #   
            # wb.close()  # if error in the above for loop, then it never reaches this line and wb never closes.
            t_returns = (ok, self.xl_out, out_pdf_final)
        except:
            msg = "{}".format(trace())
