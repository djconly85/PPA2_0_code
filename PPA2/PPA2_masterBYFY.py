...
def get_singleyr_data(fc_project, projtyp, adt, posted_speedlim, out_dict={}):
    ...
    complete_street_score = {'complete_street_score': -1} if projtyp == p.ptype_fwy else \
        cs.complete_streets_idx(p.parcel_pt_fc, fc_project, projtyp, posted_speedlim, p.trn_svc_fc)
    ...
    ag_acres = luac.get_lutype_acreage(fc_project, projtyp, p.parcel_poly_fc, p.lutype_ag)
    ...
    job_du_dens = lu_pt_buff.point_sum_density(p.parcel_pt_fc, fc_project, projtyp, 
    ...
    # get EJ data
    ej_data = lu_pt_buff.point_sum(p.parcel_pt_fc, fc_project, projtyp, [p.col_pop_ilut],
